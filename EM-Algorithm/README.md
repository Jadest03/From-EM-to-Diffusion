...
관찰가능한 확률 변수 x, 잠재변수 z, 매개변수가 $\theta = \{\phi, \mu, \Sigma \}$일 때, 로그가능도는 다음과 같이 표현할 수 있다.  
...
이때 하나의 데이터 x마다 $z = \{z^1, z^2, \dots, z^n\}$이 존재하므로 다음과 같이 작성할 수 있다.
$$
\begin{aligned}
...
### E-스텝
E-스텝에서는 KL발산 항을 0으로 만들어 고정된 $\theta = \{\phi, \mu, \Sigma \}$ 위치에서 로그가능도와  ELBO가 같아지도록 만든다.
...
E-스텝에서 구한 $q^n$을 고정하고 $\theta = \{\phi, \mu, \Sigma\}$를 갱신한다.  
