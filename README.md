...
학습 과정에서 조건을 일정 확률로 제거(Null)하여 무조건부(Unconditional) 모델과 조건부(Conditional) 모델을 동시에 학습시킨다.  
추론 시에는 두 예측값의 차이를 가이던스 스케일($\gamma$)만큼 증폭시켜, 사용자가 원하는 조건에 훨씬 더 강하게 부합하는 고품질의 결과물을 유도해 낸다.